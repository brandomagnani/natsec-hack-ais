...
def setup_postgis_geometries(
    conn: Engine, table: str, column_lat: str, column_lon: str
) -> None:
    """
    Set up PostGIS geometry columns and indexes for spatial queries.
    Executes each statement separately and handles errors.
    """
    statements = [
        ...
        f"""
        UPDATE {table} 
        SET geom = ST_SetSRID(ST_MakePoint({column_lon}, {column_lat}), 4326);
        """,
        f"""
        UPDATE {table} 
        SET geog = ST_SetSRID(ST_MakePoint({column_lon}, {column_lat}), 4326)::geography;
        """,
        # Create spatial indexes
        f"CREATE INDEX idx_{table}_geom ON {table} USING GIST (geom);",
        # Create geography index to consider earth curvature
        f"CREATE INDEX idx_{table}_geog ON {table} USING GIST (geog);",
...
def setup_additional_indices(conn: Engine) -> None:
    # create indices on timestamps and mmsis too
    statements = [
        "CREATE INDEX idx_ais_updates_timestamp ON ais_updates(timestamp);",
        "CREATE INDEX idx_ais_updates_mmsi ON ais_updates(mmsi);",
...
if __name__ == "__main__":
    ...
    conn = create_engine(f"postgresql://postgres:password@localhost:5432")
    ...
    df_landing_points.write_database(
        connection=conn, table_name="landing_points", if_table_exists="append"
    )
    df_cable_points.write_database(
        connection=conn, table_name="cable_points", if_table_exists="append"
    )
    df_ports.write_database(
        connection=conn, table_name="ports", if_table_exists="append"
    )

    all_vessel_entries = {}
    ais_files = ["/Users/felixknispel/Developer/natsec/data/AIS_2024_01_01.csv"]
    for file in tqdm(ais_files, total=len(ais_files)):
        df_ais, vessel_entries = parse_ais_data(file)
        df_ais.write_database(
            connection=conn, table_name="ais_updates", if_table_exists="append"
        )
        all_vessel_entries.update(vessel_entries)

    df_vessel_entries = pl.DataFrame(list(all_vessel_entries.values()))
    df_vessel_entries.write_database(
        connection=conn, table_name="vessels", if_table_exists="append"
    )

    setup_postgis_geometries(conn, "landing_points", "lat", "lon")
    setup_postgis_geometries(conn, "cable_points", "lat", "lon")
    setup_postgis_geometries(conn, "ports", "lat", "lon")
    setup_postgis_geometries(conn, "ais_updates", "lat", "lon")
    # vessels dont have geometry columns
    setup_additional_indices(conn)
